...
import { Flame, Sparkles, LogOut, Grid, ShieldCheck, Calendar, Trophy, Edit3, X } from "lucide-react";
    ...
    <div className="mx-auto max-w-md pb-24 pt-4 px-3 space-y-6">
      <div className="relative overflow-hidden rounded-3xl bg-dark-card border border-white/10 p-6 shadow-2xl backdrop-blur-xl text-center space-y-4">
        ...
        <div className="relative mx-auto h-24 w-24 rounded-full p-1 bg-gradient-to-tr from-fire via-neon-purple to-neon-cyan shadow-[0_0_25px_rgba(255,85,0,0.4)]">
          <img src={user?.image || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=200&auto=format&fit=crop&q=80"} alt={user?.name} className="h-full w-full rounded-full object-cover bg-dark-bg" />
        </div>
        ...
        {/* SEGUIDORES & BIO (100% autênticos com ??) */}
        <div className="space-y-2 pt-1">
          <div className="flex justify-center items-center space-x-6 text-xs">
            <div>
              <span className="font-black text-white text-base block">{user?.followersCount ?? 0}</span>
              <span className="text-[10px] text-neutral-400 uppercase font-bold">Seguidores</span>
            </div>
            <div className="h-6 w-px bg-white/10" />
            <div>
              <span className="font-black text-white text-base block">{user?.followingCount ?? 0}</span>
        ...
        <div className="grid grid-cols-3 gap-2 pt-2 border-t border-white/5">
          ...
          <div className="rounded-2xl bg-dark-elevated p-2.5 border border-white/5">
            <p className="text-white font-black text-sm">{user?.instantsCount ?? 0}</p>
            <p className="text-[9px] text-neutral-400 uppercase mt-0.5 font-bold">Instants</p>
          </div>

          <div className="rounded-2xl bg-dark-elevated p-2.5 border border-white/5">
            <p className="text-neon-purple font-black text-sm">Nvl {user?.instantsCount === 0 ? 1 : 8}</p>
        ...
        {showEditBioModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/90 backdrop-blur-xl p-6 flex flex-col justify-center text-left">
            <form onSubmit={handleSaveBio} className="space-y-4 bg-dark-card p-5 rounded-3xl border border-neon-cyan shadow-2xl">
              ...
              <p className="text-[11px] text-neutral-300">Escreva o que quiser sobre você na sua bio!</p>
      ...
      <div className="rounded-3xl bg-dark-card border border-white/10 p-5 space-y-3 shadow-xl text-left">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] text-emerald-400 font-bold bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/30">🔥 {user?.instantsCount === 0 ? "1/30" : "28/30"} Dias</span>
        </div>
        <div className="grid grid-cols-10 gap-2 pt-1">
          {retentionCalendar.map((active, idx) => (
            <div key={idx} className={`aspect-square rounded-md flex items-center justify-center text-[9px] font-mono font-bold ${active && user?.instantsCount !== 0 ? "bg-emerald-500 text-dark-bg font-black" : "bg-white/5 text-neutral-600"}`}>{idx + 1}</div>
          ))}
        </div>
      </div>
